...
class MultiTenantManager {
  ...
  login(email, senha) {
    ...
    if (usuario.cargo === 'super_admin') {
      const estabelecimentoVirtual = { id: 999, nome: "👑 SUPER ADMIN - Controle Total", plano: "enterprise", ativo: true, configuracao: { totalMesas: 999, totalComandas: 999, corTema: "purple" }, isVirtual: true };
      this.estabelecimentoAtual = estabelecimentoVirtual;
      this.salvarSessao(usuario, estabelecimentoVirtual);
      return { success: true, usuario: usuario, estabelecimento: estabelecimentoVirtual, isSuperAdmin: true, nivelAcesso: 'super_admin' };
    }
    ...
    return { success: true, usuario: usuario, estabelecimento: estabelecimento, isSuperAdmin: false, nivelAcesso: usuario.cargo };
  }

  salvarSessao(usuario, estabelecimento) {
    const sessao = { usuarioId: usuario.id, estabelecimentoId: estabelecimento.id, estabelecimentoNome: estabelecimento.nome, isSuperAdmin: usuario.cargo === 'super_admin', cargo: usuario.cargo, login: new Date().toISOString() };
  ...
  criarUsuario(dados) {
    ...
    if (!cargosPermitidos.includes(dados.cargo)) {
      return { success: false, message: `Você não pode criar usuários com cargo "${dados.cargo}". Cargos permitidos: ${cargosPermitidos.join(', ')}` };
    }

    let estabelecimentoId = dados.estabelecimentoId;
    if (usuarioAtual.cargo === 'admin') { estabelecimentoId = usuarioAtual.estabelecimentoId; }

    if (this.usuarios.some(u => u.email === dados.email)) {
      return { success: false, message: "Este email já está cadastrado!" };
    }

    const novoUsuario = { id: Date.now(), nome: dados.nome, email: dados.email, senha: dados.senha || "123456", estabelecimentoId: estabelecimentoId, cargo: dados.cargo, ativo: true, criadoPor: usuarioAtual.id, criadoEm: new Date().toISOString() };
    this.usuarios.push(novoUsuario);
    this.salvarUsuarios();
    return { success: true, usuario: novoUsuario, message: `✅ Usuário "${dados.nome}" (${dados.cargo}) criado com sucesso!` };
  }
  ...
  listarUsuarios() {
    ...
    if (usuarioAtual.cargo === 'admin') return this.usuarios.filter(u => u.estabelecimentoId === usuarioAtual.estabelecimentoId);
    return [usuarioAtual];
  }
  ...
  editarUsuario(id, dados) {
    ...
    if (usuarioAtual.cargo !== 'super_admin') {
      if (usuarioAtual.estabelecimentoId !== usuario.estabelecimentoId) return { success: false, message: "Acesso negado!" };
      if (usuario.cargo === 'admin' && usuarioAtual.id !== usuario.id) return { success: false, message: "Acesso negado! Não pode editar outro administrador." };
    }
    ...
    if (dados.cargo && usuarioAtual.cargo === 'super_admin') usuario.cargo = dados.cargo;
  ...
  toggleUsuarioStatus(id) {
    ...
    const usuario = this.usuarios.find(u => u.id === id);
    if (!usuario) return { success: false, message: "Usuário não encontrado!" };
    if (usuarioAtual.id === id) return { success: false, message: "Você não pode desativar a si mesmo!" };

    if (usuarioAtual.cargo !== 'super_admin') {
      if (usuarioAtual.estabelecimentoId !== usuario.estabelecimentoId) return { success: false, message: "Acesso negado!" };
      if (usuario.cargo === 'admin') return { success: false, message: "Acesso negado! Não pode desativar outro administrador." };
    }

    usuario.ativo = !usuario.ativo;
    this.salvarUsuarios();
    return { success: true, message: `✅ Usuário ${usuario.ativo ? 'ativado' : 'desativado'} com sucesso!` };
  }

  criarEstabelecimento(dados) {
    const usuarioAtual = this.getUsuarioAtual();
    if (usuarioAtual.cargo !== 'super_admin') return { success: false, message: "Apenas o Super Admin pode criar estabelecimentos!" };

    const novo = { id: Date.now(), nome: dados.nome, cnpj: dados.cnpj || "", endereco: dados.endereco || "", telefone: dados.telefone || "", plano: dados.plano || "basico", ativo: true, dataCadastro: new Date().toISOString(), configuracao: { totalMesas: dados.totalMesas || 10, totalComandas: dados.totalComandas || 30, corTema: dados.corTema || "emerald" } };
    this.estabelecimentos.push(novo);
    this.salvarEstabelecimentos();

    this.criarUsuario({ nome: dados.nomeAdmin, email: dados.emailAdmin, senha: dados.senhaAdmin || "123456", estabelecimentoId: novo.id, cargo: 'admin' });

    return { success: true, estabelecimento: novo, message: `✅ Estabelecimento "${dados.nome}" criado com sucesso!` };
  }

  listarEstabelecimentos() {
    const usuarioAtual = this.getUsuarioAtual();
    if (usuarioAtual.cargo === 'super_admin') return this.estabelecimentos;
    if (usuarioAtual.cargo === 'admin') return this.estabelecimentos.filter(e => e.id === usuarioAtual.estabelecimentoId);
    return [];
  }
  ...
  abrirPainelAdmin() {
    ...
    botoesParaEsconder.forEach(id => { const btn = document.getElementById(id); if (btn) btn.style.display = 'none'; });
  ...
  fecharPainelAdmin() {
    ...
    botoesParaRestaurar.forEach(id => { const btn = document.getElementById(id); if (btn) btn.style.display = ''; });
  }
  ...
  atualizarPainelAdmin() {
      ...
      tbodyEstab.innerHTML = this.estabelecimentos.map(e => `
        <tr class="border-b border-gray-700/50 hover:bg-gray-800/30">
          ...
          <td class="p-2 text-right space-x-1">
            <button onclick="tenantManager.toggleEstabelecimentoStatus(${e.id})" class="${e.ativo ? 'bg-red-700 hover:bg-red-600' : 'bg-emerald-700 hover:bg-emerald-600'} px-2 py-0.5 rounded text-[10px] text-white cursor-pointer">
    ...
    if (tbodyUser) {
      tbodyUser.innerHTML = this.usuarios.map(u => {
        ...
        const estabelecimentoNome = u.estabelecimentoId ? this.estabelecimentos.find(e => e.id === u.estabelecimentoId)?.nome || 'N/A' : 'Sistema';
        const isSuperAdmin = u.cargo === 'super_admin';
        
        let botoesAcao = '';
        if (isSuperAdmin) {
          botoesAcao = `<span class="text-gray-500 text-[10px]">👑</span>`;
        } else {
          botoesAcao = `
            <button onclick="tenantManager.abrirModalEditarUsuario(${u.id})" title="Editar usuário" class="bg-blue-600 hover:bg-blue-500 px-2.5 py-1.5 rounded text-[10px] text-white font-bold transition-colors cursor-pointer">✏️ Editar</button>
            <button onclick="tenantManager.toggleUsuarioStatus(${u.id})" title="${u.ativo ? 'Desativar usuário' : 'Ativar usuário'}" class="${u.ativo ? 'bg-red-600 hover:bg-red-500' : 'bg-emerald-600 hover:bg-emerald-500'} px-2.5 py-1.5 rounded text-[10px] text-white font-bold transition-colors cursor-pointer ml-1">
          ...
          <form id="form-editar-usuario" onsubmit="salvarEdicaoUsuario(event)" class="space-y-3">
            ...
            <div>
              ...
              <select id="edit-estabelecimento" class="w-full bg-gray-900 border border-gray-700 p-2.5 rounded text-sm text-white focus:outline-none focus:border-emerald-500">
                ${this.estabelecimentos.map(e => `<option value="${e.id}" ${usuario.estabelecimentoId === e.id ? 'selected' : ''}>${e.nome}</option>`).join('')}
              </select>
            </div>
            ...
            <div class="flex justify-end gap-2 pt-3 border-t border-gray-700">
              <button type="button" onclick="fecharModalEditarUsuario()" class="bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded text-xs font-bold text-gray-300 transition-colors">Cancelar</button>
              <button type="submit" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded text-xs font-bold text-white shadow-lg transition-colors">💾 Salvar Alterações</button>
            </div>
          </form>
        </div>
      </div>
  ...
  salvarEdicaoUsuario(e) {
    ...
    if (!id) { alert('Erro: Usuário não identificado!'); return; }
    ...
    if (!dados.nome || !dados.email) { alert('❌ Nome e email são obrigatórios!'); return; }
    if (this.usuarios.some(u => u.email === dados.email && u.id !== id)) { alert('❌ Este email já está em uso por outro usuário!'); return; }
      ...
      <div id="modal-novo-estabelecimento" class="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[100]">
        <div class="bg-gray-800 border border-gray-700 p-6 rounded-xl w-full max-w-lg space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form id="form-novo-estabelecimento" onsubmit="salvarNovoEstabelecimento(event)" class="space-y-3">
            ...
            <div class="flex justify-end gap-2 pt-3 border-t border-gray-700">
              <button type="button" onclick="fecharModalNovoEstabelecimento()" class="bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded text-xs font-bold text-gray-300 transition-colors">Cancelar</button>
              <button type="submit" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded text-xs font-bold text-white shadow-lg transition-colors">✅ Criar Estabelecimento</button>
            </div>
          </form>
        </div>
      </div>
  ...
  salvarNovoEstabelecimento(e) {
    ...
    if (!dados.nome) { alert('❌ O nome do estabelecimento é obrigatório!'); return; }
    if (!dados.nomeAdmin || !dados.emailAdmin) { alert('❌ Os dados do administrador são obrigatórios!'); return; }
  ...
  abrirModalNovoUsuario() {
    const usuarioAtual = this.getUsuarioAtual();
    const cargosPermitidos = this.getCargosPermitidos();
    if (cargosPermitidos.length === 0) { alert('❌ Você não tem permissão para criar usuários!'); return; }
      ...
      <div id="modal-novo-usuario" class="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[100]">
        <div class="bg-gray-800 border border-gray-700 p-6 rounded-xl w-full max-w-md space-y-4 shadow-2xl">
          ...
          <form id="form-novo-usuario" onsubmit="salvarNovoUsuario(event)" class="space-y-3">
            ...
            ` : ''}
            <div class="flex justify-end gap-2 pt-3 border-t border-gray-700">
              <button type="button" onclick="fecharModalNovoUsuario()" class="bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded text-xs font-bold text-gray-300 transition-colors">Cancelar</button>
              <button type="submit" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded text-xs font-bold text-white shadow-lg transition-colors">✅ Criar Usuário</button>
            </div>
          </form>
        </div>
      </div>
  ...
  salvarNovoUsuario(e) {
    e.preventDefault();
    const usuarioAtual = this.getUsuarioAtual();
    const dados = { nome: document.getElementById('nu-nome').value.trim(), email: document.getElementById('nu-email').value.trim(), senha: document.getElementById('nu-senha').value.trim() || '123456', cargo: document.getElementById('nu-cargo').value, estabelecimentoId: null };

    if (usuarioAtual.cargo === 'super_admin') {
      const estabSelect = document.getElementById('nu-estabelecimento');
      if (estabSelect) dados.estabelecimentoId = parseInt(estabSelect.value);
    } else {
...
window.abrirPainelAdmin = function() { tenantManager.abrirPainelAdmin(); };
window.fecharPainelAdmin = function() { tenantManager.fecharPainelAdmin(); };
window.atualizarPainelAdmin = function() { tenantManager.atualizarPainelAdmin(); };
window.fecharModalEditarUsuario = function() { tenantManager.fecharModalEditarUsuario(); };
window.salvarEdicaoUsuario = function(e) { tenantManager.salvarEdicaoUsuario(e); };
window.abrirModalNovoEstabelecimento = function() { tenantManager.abrirModalNovoEstabelecimento(); };
window.fecharModalNovoEstabelecimento = function() { const modal = document.getElementById('modal-novo-estabelecimento'); if (modal) modal.remove(); };
window.salvarNovoEstabelecimento = function(e) { tenantManager.salvarNovoEstabelecimento(e); };
window.abrirModalNovoUsuario = function() { tenantManager.abrirModalNovoUsuario(); };
window.fecharModalNovoUsuario = function() { const modal = document.getElementById('modal-novo-usuario'); if (modal) modal.remove(); };
window.salvarNovoUsuario = function(e) { tenantManager.salvarNovoUsuario(e); };
...
window.logoutMulti = function() {
  ...
  document.body.classList.remove('super-admin-mode');
  document.querySelectorAll('nav button').forEach(btn => { btn.style.display = ''; });
  if (typeof usuarioLogado !== 'undefined') { window.usuarioLogado = null; }
