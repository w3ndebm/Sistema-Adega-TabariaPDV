...
function abrirPainelAdmin() {
  ...
  // Carregar conteúdo
  if (tenantManager && typeof tenantManager.atualizarPainelAdmin === 'function') {
    tenantManager.atualizarPainelAdmin();
  } else {
    // Fallback: carregar conteúdo manualmente
    const conteudo = document.getElementById('conteudo-painel-admin');
    if (conteudo) {
      conteudo.innerHTML = `
        <div class="text-center py-8">
          <p class="text-emerald-400 text-xl font-bold">👑 Painel Admin</p>
          <p class="text-gray-400 text-sm mt-2">Carregando dados...</p>
          <button onclick="tenantManager.atualizarPainelAdmin()" class="mt-4 bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded text-xs font-bold text-white">
            🔄 Carregar Dados
          </button>
        </div>
      `;
